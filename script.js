...
function switchOutingSubTab(type) {
    ...
    if (type === 'event' && currentRegion !== 'all') {
    ...
    body.innerHTML = `
        ...
        <div class="modal-meta-box">
            <div class="modal-meta-row">
                <span class="modal-meta-label">🗓️ 기간</span>
                <span class="modal-meta-value">${dateText}</span>
            </div>
            <div class="modal-meta-row">
                <span class="modal-meta-label">📍 장소</span>
                <span class="modal-meta-value">${addr}</span>
            </div>
        </div>

        <div class="place-review" style="margin-top:0; margin-bottom:20px; background:#F2F5F8; border-radius:14px;">
            <strong>💬 토실이 팩트 체크:</strong> "${review || '공식 지자체 엄선 아동 가족 맞춤형 주말 안전 인프라 축제입니다.'}"
        </div>
        ...
        <div class="modal-navi-container">
            <a href="${naverUrl}" target="_blank" class="modal-navi-item">
                <div class="navi-badge-icon naver">N</div>
                <span>네이버 지도</span>
            </a>
            <a href="${tmapUrl}" target="_blank" class="modal-navi-item">
                <div class="navi-badge-icon tmap">TMAP</div>
                <span>티맵(TMap)</span>
            </a>
            <a href="${kakaoUrl}" target="_blank" class="modal-navi-item">
                <div class="navi-badge-icon kakao">K</div>
                <span>카카오내비</span>
            </a>
        </div>
        ...
        <div class="modal-action-grid">
            ${telLink}
            <button class="btn-main" style="margin-top:0; padding:16px; border-radius:14px; background:var(--text-m) !important;" onclick="closeFestivalModalForce()">확인 완료</button>
        </div>
    `;
...
function closeFestivalModalForce() { document.getElementById('premium-modal').style.none || (document.getElementById('premium-modal').style.display = 'none'); }
        ...
        filteredEvents.forEach(item => {
            ...
            card.innerHTML = `
                <div class="fest-card-img-wrap">
                    <span class="fest-dday-tag">🎉 축제</span>
                    ${imgHtml}
                </div>
                <div class="fest-card-info">
                    <div class="fest-card-title">${title}</div>
                    <div class="fest-card-meta">${shortAddr}</div>
                </div>
            `;
        ...
        filteredPlaces.forEach((p, index) => {
            ...
            <div class="accordion-item">
                ...
                <div id="acc-body-${index}" class="accordion-body">
                    ...
                    <button class="btn-main" style="margin-top:16px; font-size:14px; padding:12px;" onclick="openFestivalModal('${p.title}', '${p.datetime || '상시 운영'}', '${p.locText}', '정보없음', '${p.review}', '${p.query}', '')">
                        🚗 내비게이션 길찾기 및 상세 정보 열기 〉
                    </button>
                </div>
            </div>`;
...
function setRegion(region, btn) {
    ...
    if (currentSubTab === 'event' && region !== 'all') {
...
function promptBabyInfo() {
    ...
    if(!name) return;
    const birth = prompt("아기 생년월일을 입력하세요 (YYYY-MM-DD)", "2025-11-15");
    if(!birth) return;
    localStorage.setItem('tosil_baby', JSON.stringify({name, birth}));
    renderBabyInfo();
}
...
function renderBabyInfo() {
    ...
    if(!saved) return;
    const {name, birth} = JSON.parse(saved);
    ...
    if(nameEl) nameEl.innerText = name + "의 공간";

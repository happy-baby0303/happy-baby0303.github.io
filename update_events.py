...
def update_homepage():
    ...
    # 🛠️ 버그 수정 핵심: 정규식 대신 한 줄씩 쪼개서 안전하게 고정 육아지도만 발라냅니다.
    place_blocks = []
    for line in original_array_content.split('\n'):
        if 'isEvent: false' in line:
            cleaned_line = line.strip().rstrip(',')
            if cleaned_line:
                place_blocks.append(cleaned_line)
    
    print(f"📍 파트너님의 오리지널 육아지도 데이터 {len(place_blocks)}곳 안전 보존 완료.")
